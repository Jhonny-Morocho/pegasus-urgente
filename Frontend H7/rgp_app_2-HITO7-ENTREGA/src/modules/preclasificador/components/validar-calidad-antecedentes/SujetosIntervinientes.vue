...
import ModalSolicitarContactarVictima from '../modals/ModalSolicitarContactarVictima.vue'
...
const { ruc, idDenuncia } = useCausaPreclasificadorStore()
...
const { 
  ...
  resetFormData, 
  resetByKeys 
} = useFormValidation({ formData, validationScope: sujetosIntervinientesValidationScope })
...
function eliminarSujeto (sujeto) {
  const nombreCompleto = `${sujeto.nombres} ${sujeto.apellidoPaterno} ${sujeto.apellidoMaterno}`
  launchConfirmDialog({
    message: `¿Estás seguro(a) de eliminar a ${nombreCompleto}`,
    accept: async () => {
      requestUtil({
        ...
        resolve: async () => {
          toastSuccess(`Se eliminó el interviniente ${nombreCompleto}`)
          flujoPreclasificador.cambiarTipificacionOtro = true
          getSujetos()
        }
      })
