...
function getRelatoById () {
  requestUtil({
    ...
    resolve: (r) => console.log('relato: ', r),
    reject: (e) => {
...
function guardarRelato (data) {
  console.log('guardarRelato', procesoPam.value)
  data.idDenuncia = procesoPam.value.idDenuncia
  data.idProcedimiento = procesoPam.value.idCausa
  data.idFuncionarioRol = procesoPam.value.idFuncionarioRol ?? appUtil.cadenaAleatoria(20)

  if (data.idRelato?.length === 36) {
    // OJO ACA
    requestUtil({
...
function handleSubmitForm () {
  if (relato.value?.relato) {
    ...
    setTimeout(() => {
      ...
      emit('next-step', '/ingreso/denuncia-fisica/nueva-denuncia/documentos')
    }, 600)
