...
import { useBandejaFiscalRegional, useTermino } from '../composables'
...
const { bandejaFiscalRegional_getDataBandeja, causas, paginacion } =
  useBandejaFiscalRegional()
...
onMounted(() => {
  ...
  const grupoFiscal = roles.value.find((r) =>
    r.includes(`${fiscalia}_encargado_postermino_TPT`)
  )
...
async function getDataBandeja (pagination = undefined) {
  ...
  requestUtil({
    action: bandejaFiscalRegional_getDataBandeja,
    payload: {
      params: {
        ...fiscal.filters,
...
const iniciarTarea = ({ data: { causa }, tarea }) => {
  ...
  if (
    taskName === 'Registrar Modificaciones'.replace(/\s/g, '').toUpperCase()
  ) {
    router.push({ name: 'RegistrarModificacionAR' })
  } else if (
    taskName ===
    'Registrar Cumplimiento/  Incumplimiento'.replace(/\s/g, '').toUpperCase()
  ) {
    router.push({ name: 'RegistrarCumplimiento' })
