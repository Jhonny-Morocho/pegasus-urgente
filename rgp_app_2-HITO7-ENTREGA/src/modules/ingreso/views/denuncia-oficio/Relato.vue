...
import { useRelatoStore, usePamStore, useMantenedorStore } from '@/modules/global/composables'
import { appUtil, requestUtil } from '@/utils'
import { defineEmits, ref, onMounted } from 'vue'

const { toastSuccess, toastError, toastWarning } = useAppToast()

const { getIntervinientes, getEspecies, getDelitos, relato_getPropuestaRelatoById, relato_guardarRelato, relato_editarRelato, relato_getRelatoByIdDenuncia, relato_getPropuestaPlantilla, relato } = useRelatoStore()
...
function guardarRelato (data) {
  ...
  console.log('payload relato', data)
...
function handleSubmitForm () {
  if (relato?.value) {
    ...
    setTimeout(() => {
      ...
      emit('next-step', '/ingreso/denuncia-oficio/nueva-denuncia/documentos')
    }, 600)
