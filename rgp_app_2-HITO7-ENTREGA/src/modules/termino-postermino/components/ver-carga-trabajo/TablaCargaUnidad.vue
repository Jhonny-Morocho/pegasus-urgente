  ...
  <Column header="Total Causas con Acuerdo Reparatorio">
    <template #body="{ data }">
      <span @click="openTotalCausas(data)">
        {{ data.totalCausasAcuerdos }}
      </span>
    </template>
  </Column>
  <Column header="Tiempo Promedio Tramitar Causa (días).">
    <template #body="{ data }">
      {{ data.promedioDiasTramitar }}
    </template>
  </Column>
