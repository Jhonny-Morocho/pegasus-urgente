...
const {
  ...
  idDenuncia,
  resumenCausa,
  dirigirInvestigacion_postCompletarCierreInvestigacionEscrito
...
const {
  ...
  paginacion: paginacionDocumentos
} = useDocumentoStore()
...
async function completarCierreEscrito () {
  const relacionesParse = relaciones.value?.map((el) => el.idRelacionDelito)

  const payloadParse = {
    idCausa: idCausa.value,
    idUnidadOrigen: resumenCausa.value?.idFiscalia,
    idUnidadDestino: formData?.nombreTribunal,
    instruccionFiscal: 'string',
    seDerivaEquipoJuridico: formData?.derivaMaterializacionEquipoJuridico,
    seRevisaEscrito: formData?.derivaMaterializacionEquipoJuridico,
    idPlantilla: null,
    textoHito: formData?.materializacionTextoFiscal,
    idsRelacionDelito: relacionesParse,
    idsSujetoTestigo: null,
    idsDocumento: null
  }

  await requestUtil({
    action: dirigirInvestigacion_postCompletarCierreInvestigacionEscrito,
...
const getPlantilla = (id) => {
  const plantilla = plantillasDiligencia.value.find(
