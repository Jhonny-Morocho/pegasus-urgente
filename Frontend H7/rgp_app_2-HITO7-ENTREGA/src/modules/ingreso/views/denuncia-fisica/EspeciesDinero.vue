...
<script setup>
import { defineEmits, onMounted, provide, reactive, ref } from 'vue'
...
const { especiesFormData, especiesDineroValidationScope } =
  useEspeciesDineroProvide()
provide(especiesDineroValidationScope, especiesFormData)

const { formSubmitted, isFormValid, resetFormData } =
  useFormValidation({
...
const esGenerico = (data) => {
  return (
    !data.esVehiculo &&
    !data.esDinero &&
    !data.existeAnimalInvolucrado &&
    !data.esDocumentoValor
  )
}
...
function handleNextStep () {
  ...
  setTimeout(() => {
    ...
    emit('next-step', '/ingreso/denuncia-fisica/nueva-denuncia/relato')
  }, 600)
}
...
function getDocumentosEnSeccion (paginacion = undefined) {
  if (!idCatalogo.value) {
