...
<script setup>
import { defineEmits, onMounted, provide, reactive, ref, nextTick } from 'vue'
...
import { useDelitoStore, useDocumentoStore, usePamStore, useMantenedorStore } from '@/modules/global/composables'
...
function guardarDelito () {
  ...
  if (!editando.value && !delitosPag.isLoadingTable) {
    ...
    requestUtil({
      ...
      reject: () => {
        toastError('Error, el delito no se pudo guardar, intenta nuevamente')
      }
    })
  }
...
function handleNextStep () {
  if (delitos.value?.length > 0) {
    ...
    setTimeout(() => {
      ...
      emit('next-step', '/ingreso/denuncia-oficio/nueva-denuncia/relaciones')
    }, 600)
  } else {
